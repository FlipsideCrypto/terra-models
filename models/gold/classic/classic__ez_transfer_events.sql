...
SELECT DISTINCT
  ...
  symbol
FROM {{ ref('classic__dim_oracle_prices') }}
qualify(ROW_NUMBER() over (PARTITION BY currency
ORDER BY
...
FROM
  {{ ref('silver_classic__transfer_events') }} t
