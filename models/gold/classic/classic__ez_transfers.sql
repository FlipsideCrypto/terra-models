...
FROM {{ ref('silver_classic__msg_events') }} r,
...
LEFT OUTER JOIN {{ ref('silver_classic__address_labels') }} AS from_labels
  ON event_from = from_labels.address 
  AND from_labels.blockchain = 'terra' 
  AND from_labels.creator = 'flipside'

LEFT OUTER JOIN {{ ref('silver_classic__address_labels') }} AS to_labels
