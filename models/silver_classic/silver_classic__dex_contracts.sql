{{ config(
  ...
  tags = ['snowflake', 'classic', 'dex_contracts', 'terra_contracts']
) }}
...
FROM {{ ref('silver_classic__msg_events') }} me
JOIN {{ ref('silver_classic__address_labels') }} al ON me.event_attributes:"0_contract_address"::string = al.address
WHERE
event_type = 'wasm'
...
LEFT OUTER JOIN {{ ref('silver_classic__contract_metadata') }} cm1 ON ce.token1 = cm1.contract_address
LEFT OUTER JOIN {{ ref('silver_classic__contract_metadata') }} cm2 ON ce.token2 = cm2.contract_address
